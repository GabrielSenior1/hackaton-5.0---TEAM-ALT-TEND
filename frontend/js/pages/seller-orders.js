...
const estadoConfig = {
  pendiente: { label: 'Pendiente', icon: 'schedule', color: 'secondary', badgeClass: 'status-badge--pending' },
  confirmado: { label: 'Confirmado', icon: 'check_circle', color: 'tertiary', badgeClass: 'status-badge--active' },
  enviado: { label: 'Enviado', icon: 'local_shipping', color: 'primary', badgeClass: 'status-badge--exported' },
  entregado: { label: 'Entregado', icon: 'done_all', color: 'tertiary', badgeClass: 'status-badge--active' },
  cancelado: { label: 'Cancelado', icon: 'cancel', color: 'error', badgeClass: '' },
};
    ...
    <div class="seller-content">
      <div class="container" style="padding: 32px 24px; display: flex; flex-direction: column; gap: 28px; max-width: 1100px;">
        ...
        <section style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
          <div>
            <h2 class="headline-lg" style="color: var(--on-background);">Pedidos</h2>
            <p class="body-md" style="color: var(--on-surface-variant); margin-top: 4px;">Gestiona los pedidos de tus compradores</p>
          </div>
        ...
        <div id="seller-orders-list" style="display: flex; flex-direction: column; gap: 16px;">
          <div style="text-align: center; padding: 60px 20px;">
            ...
            <p class="body-md" style="color: var(--on-surface-variant);">Cargando pedidos...</p>
          </div>
        </div>
  ...
  try {
    ...
    if (list) list.innerHTML = `
      <div style="text-align: center; padding: 40px;">
        ...
        <p class="body-md" style="color: var(--error); margin-top: 8px;">Error cargando pedidos</p>
      </div>
    `;
  }
}
    ...
    list.innerHTML = `
      <div style="text-align: center; padding: 60px 20px;">
        ...
        <p class="body-md" style="color: var(--on-surface-variant); margin-top: 12px;">
          ${orderFilter === 'all' ? 'No tienes pedidos aún' : 'No hay pedidos con este estado'}
        </p>
      </div>
    `;
    return;
  ...
  list.innerHTML = filtered.map(order => {
    ...
    return `
      <div class="card" style="padding: 20px; display: flex; flex-direction: column; gap: 16px; animation: fadeInUp 0.4s ease;">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
          <div>
            <p style="font-size: 12px; color: var(--on-surface-variant);">Comprador: <strong>${order.compradorNombre || order.compradorEmail || 'Anónimo'}</strong></p>
          </div>
  ...
  list.querySelectorAll('.order-action-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      try {
        ...
        renderOrdersList();
        window.__components?.showToast?.(`Pedido actualizado a: ${estadoConfig[btn.dataset.action]?.label}`, 'success');
      } catch (e) {
        window.__components?.showToast?.('Error actualizando pedido', 'error');
      }
    });
  });
}
